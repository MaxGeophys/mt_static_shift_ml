...
амплитудных кривых МТЗ с применением двух методик нормализации: улучшенной версии низкочастотной пространственной фильтрации и
...
являются: `Rho_mean_ratio` --- отношение величины в заданной точке к
...
Например, для `Zyx` это `Zyx_mean_ratio`, `Rho`, `absZyx`, `absZyy`,
...
используются `Zxx_mean_ratio`, `absZxx`, `imagZxx`, `AlphaEgg2`, а
...
Итоговые метрики для эффективного кажущегося сопротивления `Rho`
на тестовой геоэлектрической модели типа "Грабен":

| Метод                                       | RMSE | MAE | R² | MAPE |
|---------------------------------------------|---:|---:|---:|---:|
| Модель с приповерхностными неоднородностями | 96.90 | 39.20 | 0.065 | 43.69% |
| Результат пространственной фильтрации       | 38.60 | 22.66 | 0.852 | 31.70% |
| Применение градиентного бустинга            | **23.58** | **10.77** | **0.945** | **12.47%** |

Градиентный бустинг показывает лучшие результаты по всем приведённым
метрикам: RMSE снижается с 96.90 Ом·м для исходных искажённых данных
до 23.58 Ом·м после нормализации.

Ниже приведён пример результатов площадной нормализации эффективного кажущегося
сопротивления на периоде T = 200 с.

![Comparison of Rho normalization methods at T = 200 s](visualization/examples/Rho_T_200s.png)

Показаны:
- **a** — эталонная модель с однородным приповерхностным слоем;
- **b** — модель с множеством приповерхностных неоднородностей;
- **c** — результат пространственной фильтрации;
- **d** — результат градиентного бустинга.

На периоде T = 200 с RMSE исходных искажённых данных составляет
192 Ом·м. Пространственная фильтрация снижает RMSE до 75.93 Ом·м,
а градиентный бустинг — до 46.93 Ом·м.
